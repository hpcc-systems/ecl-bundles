EXPORT SpellCheck := MODULE,FORWARD  
  ...
  END;
	...
	//
	//    2.  function fn_build_spellcheck_files: builds the necessary base files and keyfiles 
	...
	SHARED fn_StringSplit(STRING str, STRING delimiter = ' ') :=
		FUNCTION
			// In.
			inrec       := {STRING text};
			str_trimmed := TRIM(str,LEFT,RIGHT);
			ds_str      := DATASET( [{str_trimmed}], inrec );
			// Out.
			outrec      := {STRING word};
			norm_lettrs := NORMALIZE(ds_str, LENGTH(str_trimmed), TRANSFORM( outrec, SELF.word := LEFT.text[COUNTER] ) );
			result_pre  := ROLLUP( norm_lettrs, RIGHT.word != delimiter, TRANSFORM( outrec, SELF.word := LEFT.word + RIGHT.word ) );
			result      := PROJECT( result_pre, TRANSFORM(outrec, SELF.word := StringLib.StringFilterOut(LEFT.word,delimiter)) );
			RETURN result;
		END;
	...
	EXPORT Checker(STRING strDocument, STRING1 delimiter = ' ') := MODULE
		...
		alphachars_plus_delimiter := 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ' + delimiter;
		strDocument_cleaned := STD.Str.Filter( strDocument, alphachars_plus_delimiter );
		...
		SHARED ds_spelling_suggestions_ungrouped :=
			JOIN(
				ds_words_misspelled, key_misspellings,
	...
	EXPORT fn_build_spellcheck_files(STRING import_wordlist_filename = '') :=
		FUNCTION
