...
Cmat1 := tm.Matrix(N1, M1, 1.0, 1);
Cmat2 := tm.Matrix(M1, N1, 1.0, 2);
alpha := -.9;



newmat := PBblas.axpy(alpha, Amat1+Amat2, Cmat1+Cmat2);
new1 := newmat(wi_id = 1);
new2 := newmat(wi_id = 2);
...
tp2_x := 23;
tp2_y := 47;
Aval1_1  := Amat1(x=tp1_x AND y=tp1_y)[1].v;
Cval1_1  := Cmat1(x=tp1_x AND y=tp1_y)[1].v;
Aval1_2  := Amat1(x=tp2_x AND y=tp2_y)[1].v;
Cval1_2  := Cmat1(x=tp2_x AND y=tp2_y)[1].v;
Aval2_1  := Amat2(x=tp1_x AND y=tp1_y)[1].v;
Cval2_1  := Cmat2(x=tp1_x AND y=tp1_y)[1].v;
Aval2_2  := Amat2(x=tp2_x AND y=tp2_y)[1].v;
Cval2_2  := Cmat2(x=tp2_x AND y=tp2_y)[1].v;

expect1_1 := alpha * Aval1_1 + Cval1_1;
expect1_2 := alpha * Aval1_2 + Cval1_2;
expect2_1 := alpha * Aval2_1 + Cval2_1;
expect2_2 := alpha * Aval2_2 + Cval2_2;
